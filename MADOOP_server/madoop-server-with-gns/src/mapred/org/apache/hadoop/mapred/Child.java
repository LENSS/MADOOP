...
class Child {
  ...
  public static void innerChild(TaskRunner tr, File workDir) throws IOException {
    LOG.debug("Child starting");

    JobConf defaultConf = new JobConf();
    TaskTracker tracker = tr.getTracker();
    taskid = tr.getTask().getTaskID();

    InetSocketAddress temp = tracker.getTaskTrackerReportAddress();
    String host = temp.getAddress().getHostAddress(); 
    int port = Integer.parseInt(Integer.toString(temp.getPort()));
    InetSocketAddress address = new InetSocketAddress(host, port);
    final TaskAttemptID firstTaskid = TaskAttemptID.forName(taskid.toString());
...
//        }
        ...
        TaskRunner.setupWorkDir(job, workDir);
//        File workDir = new File(".").getAbsoluteFile();
        LOG.info("<innerChild> local workDir: " + workDir.getAbsoluteFile());
...
//        JvmMetrics.init(task.getPhase().toString(), job.getSessionId());
        ...
        LOG.info("<innerChild> DFS workDir: " + job.getWorkingDirectory());
  ...
  public static void main(String[] args) throws Throwable {
    ...
    try {
      while (true) {
        ...
        File workDir = new File(".").getAbsoluteFile();
        LOG.info("<main> local workDir: " + workDir.getAbsoluteFile());
        ...
        JvmMetrics.init(task.getPhase().toString(), job.getSessionId());
        // use job-specified working directory
        LOG.info("<main> DFS workDir: " + job.getWorkingDirectory());
        FileSystem.get(job).setWorkingDirectory(job.getWorkingDirectory());
        try {
          task.run(job, umbilical);             // run the task
        } finally {

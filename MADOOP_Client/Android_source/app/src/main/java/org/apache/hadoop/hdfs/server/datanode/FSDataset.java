...
public class FSDataset implements FSConstants, FSDatasetInterface {
  public static final ILogger LOG = LogFactory.getLog(FSDataset.class);
  ...
  private BlockWriteStreams createBlockWriteStreams( File f , File metafile) throws IOException {
      return new BlockWriteStreams(f, new FileOutputStream(new RandomAccessFile( f , "rw" ).getFD()),
  ...
  public BlockWriteStreams writeToBlock(Block b, boolean isRecovery) throws IOException {
    ...
    synchronized (this) {
      ...
      if (!isRecovery) {
        ...
        LOG.info("****<writeToBlock> writeToBlockReopen already-open Block for append " + b);
        // create or reuse temporary file to hold block in the designated volume
        v = volumeMap.get(b).getVolume();
        volumeMap.put(b, new DatanodeBlockInfo(v));
      } else {
        // reopening block for appending to it.
        LOG.info("****<writeToBlock> writeToBlockReopen Block for append " + b);
        v = volumeMap.get(b).getVolume();
        f = createTmpFile(v, b);
        ...
        LOG.info("****<writeToBlock> Renaming " + oldmeta + " to " + newmeta);
        ...
        LOG.info("****<writeToBlock> Renaming " + blkfile + " to " + f);
      ...
      if (f == null) {
        LOG.warn("****<writeToBlock> Block " + b + " reopen failed " +
    ...
    LOG.info("****<writeToBlock> writeTo blockfile is " + f + " of size " + f.length());
    LOG.info("****<writeToBlock> writeTo metafile is " + metafile + " of size " + metafile.length());
...
//    LOG.info("****<getChannelPosition> file: " + file);
    LOG.info("****<getChannelPosition> file.getChannel(): " + file.getChannel());
